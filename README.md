...
* **🧠 智能线路记忆**: 
    * **自动保存偏好**: 针对支持多线路的频道，系统会根据 `tvg-name` 或频道名自动记忆你上次选择的线路。
    * **失效回退机制**: 如果 M3U 更新后原线路丢失，系统会自动回退至默认线路，并弹出 Toast 友情提示。
* **🔍 深度优化的 M3U 解析**:
    * **4K 保护逻辑**: 特别优化正则解析，确保 `CCTV 4K`、`576` 分辨率标识等频道名不会被错误裁剪。
    * **物理顺序对齐**: 频道列表严格遵循 M3U 文件中的原始物理顺序，不再出现乱序问题。
    * **多源自动聚合**: 相同分组下的同名频道会自动合并为单一入口，通过右侧“i”按钮即可轻松切源。
...
### 2. 交互逻辑
* **点击频道**: 直接播放。系统会自动加载你上次偏好的线路。
* **点击右侧 (i) 按钮**: 弹出“线路配置”菜单，你可以在这里切换备用源并保存偏好。
